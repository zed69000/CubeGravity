...
function beep(freq=440,dur=.08,type="square",gain=.04,kind="sfx"){
  if(kind==="sfx" && (!soundOn || !sfxOn))return;
  if(kind==="music" && (!soundOn || !musicOn))return;
  const ac=ensureAudio(); if(!ac)return;
  const o=ac.createOscillator(), g=ac.createGain(), f=ac.createBiquadFilter();
  o.type=type; o.frequency.value=freq;
  f.type="lowpass"; f.frequency.value=kind==="music"?1200:2200;
...
function startElevatorMusic(){
  if(elevatorMusicStarted || !soundOn || !musicOn || gamePaused)return;
...
function userAudioGesture(){ensureAudio(); if(bgMusic)bgMusic.load(); if(musicOn)startElevatorMusic();}
...
document.getElementById("pauseSfxToggle")?.addEventListener("click",()=>{sfxOn=!sfxOn;soundOn=sfxOn||musicOn;updatePauseButtons();});
document.getElementById("pauseMusicToggle")?.addEventListener("click",()=>{musicOn=!musicOn;soundOn=sfxOn||musicOn;updatePauseButtons();if(musicOn&&!gamePaused)startElevatorMusic();else stopElevatorMusic();});
