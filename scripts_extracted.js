...
function resizeCurrentLevelTo(size){
  ...
  if(!lvl) return;
  const oldMap=map();
  lvl.gridSize=clampGridSize(size);
  ...
  resetPlay();
  setStatus("Grille niveau : "+W+"×"+H+". Les blocs ont été réadaptés. Oui, ça bouge sans tout détruire, miracle.");
}
...
function normalizeLevelMapSize(map){
  ...
  const oldH=rows.length, oldW=Math.max(0,...rows.map(r=>r.length));
  for(let y=0;y<oldH;y++){
    for(let x=0;x<(rows[y]||"").length;x++){
      const ch=rows[y][x];
      if(!ch || ch===".") continue;
      const nx=Math.max(0,Math.min(targetW-1, x-Math.floor((oldW-targetW)/2)));
      const ny=Math.max(0,Math.min(targetH-1, y-Math.floor((oldH-targetH)/2)));
      if(out[ny][nx]===".") out[ny][nx]=ch;
      else {
        let placed=false;
        for(let r=1;r<Math.max(targetW,targetH)&&!placed;r++){
          for(let yy=Math.max(0,ny-r);yy<=Math.min(targetH-1,ny+r)&&!placed;yy++){
            for(let xx=Math.max(0,nx-r);xx<=Math.min(targetW-1,nx+r);xx++){
              if(out[yy][xx]==="."){out[yy][xx]=ch;placed=true;break;}
            }
          }
        }
      }
...
let door=false,busy=false,edit=!!window.CG_EDITOR_MODE,moves=0,gameOver=false,confettiTimer=null,deadPlayer=false,idleTick=0,painting=false,lastPaintKey='',visualOri=0,squash=0,soundOn=true,audioCtx=null,rotatorCooldown=false;
let wallNoteIndex=0,lastWallSoundAt=0,eternalFallWraps=0,elevatorMusicTimer=null,elevatorMusicStarted=false;
...
function resetPlay(){
  ...
  door=baseDoorOpen();
  moves=0;busy=false;gameOver=false;deadPlayer=false;rotatorCooldown=false;eternalFallWraps=0;activeSwitchKey=null;activeSwitchHeld=false;hideBanner();
  ...
  draw(); updateStats();
  if(!edit){busy=true;setTimeout(fall,getFallDelayMs());scheduleCompanion(false)}
}
function updateStats(){
  const max=levels[currentLevel].maxMoves||"?";
  const name=levels[currentLevel].name||("Niveau "+(currentLevel+1));
  if(edit){
    playStats.textContent="";
  }else{
    playStats.innerHTML =
      '<div class="objectiveLine">'+
  ...
  if(badge)badge.textContent=(currentLevel+1)+" - "+name;
}
...
function startElevatorMusic(){
  if(elevatorMusicStarted || !soundOn)return;
  elevatorMusicStarted=true;
  const playBar=()=>{
    if(!soundOn)return;
    const chord=[261.63,329.63,392.00,493.88];
    chord.forEach((f,i)=>setTimeout(()=>beep(f,.34,"sine",.008),i*95));
    setTimeout(()=>beep(220,.42,"triangle",.006),360);
  };
  playBar();
  elevatorMusicTimer=setInterval(playBar,1650);
}
...
function soundMove(){beep(210,.045,"square",.018)}
function soundRotate(d){
  if(d>0){beep(260,.055,"triangle",.035);setTimeout(()=>beep(360,.07,"sine",.03),65)}
  else{beep(360,.055,"triangle",.035);setTimeout(()=>beep(240,.07,"sine",.03),65)}
}
function soundLand(d=1){
  const now=performance.now();
  if(now-lastWallSoundAt<90)return;
  lastWallSoundAt=now;
  const notes=[196,220,247,294,330,392,440,523];
  const f=notes[wallNoteIndex++%notes.length] + Math.min(d,6)*4;
  beep(f,.12,"sine",.045);
  setTimeout(()=>beep(f*1.5,.08,"triangle",.022),55);
}
function soundCrash(){beep(70,.18,"sawtooth",.06);setTimeout(()=>beep(45,.18,"square",.04),70)}
function soundWin(){
  [523,659,784,1046].forEach((f,i)=>setTimeout(()=>beep(f,.13,"triangle",.045),i*90));
  for(let i=0;i<12;i++)setTimeout(()=>beep(700+Math.random()*600,.06,"sine",.018),220+i*55);
}
function soundLove(){beep(523,.08,"sine",.04);setTimeout(()=>beep(784,.12,"sine",.04),100)}
function soundLose(){beep(392,.16,"triangle",.045);setTimeout(()=>beep(370,.18,"triangle",.04),180);setTimeout(()=>beep(330,.45,"sawtooth",.035),390)}
function soundDoor(){beep(180,.06,"square",.035);setTimeout(()=>beep(420,.12,"triangle",.032),70);}
function soundEternalFall(){beep(160,.16,"sawtooth",.045);setTimeout(()=>beep(120,.22,"sine",.04),160);setTimeout(()=>beep(90,.35,"triangle",.035),360)}
function userAudioGesture(){ensureAudio();startElevatorMusic();}
function toggleSound(){
  soundOn=!soundOn;
  document.getElementById("soundToggle").textContent=soundOn?"🔊 Son":"🔇 Son";
...
function showBanner(title,msg,stars,kind){hideCompanion(false);
...
function appendVectorLayer(parent,l,t){
  ...
  if(hasScale||hasRot){
    const cx=Number(l.pivotX ?? ((Number(l.x??0)+Number(l.w??0)/2)||20));
    const cy=Number(l.pivotY ?? ((Number(l.y??0)+Number(l.h??0)/2)||20));
...
function layerBBox(l){
  ...
  if(l.shape==="text"){return {x:Number(l.x??0),y:Number(l.y??0)-Number(l.fontSize??12),w:String(l.text??"").length*Number(l.fontSize??12)*0.6,h:Number(l.fontSize??12)};}
...
function draw(){
  ...
  for(let y=0;y<H;y++)for(let x=0;x<W;x++){
    ...
    if(c=="B"){
      if(drawVectorAsset("destination",X,Y)){}else if(!drawPixelAsset("destination",X,Y)){
        rect(X+4,Y+4,T-8,T-8,"#ff4f93","#8a1745");
      }
    }
    if(c=="A"&&edit){
      if(drawVectorAsset("player",X,Y)){}else if(!drawPixelAsset("player",X,Y)){
  ...
  if(!edit&&!deadPlayer){
    ...
    if(drawVectorAsset("player",0,0,grp)){}else if(!drawPixelAsset("player",0,0,grp)){
...
function fall(){
  ...
  const step=()=>{
    if(edit||gameOver){busy=false;return}
    if(safety++>160){
      busy=false;
...
function calcMoves(){
  try{
    ...
    if(start.dead){
      ...
      status.innerHTML='<span class="testBad">Calc coups : impossible dès le départ ('+start.reason+').</span>';
      updateStats();
      return;
    }
    let q=[{...start,path:[]}];
    ...
    while(q.length){
      ...
      if(s.p.x==s.ex.x && s.p.y==s.ex.y){
        ...
        updateStats();
        status.innerHTML='<span class="testOk">Calc coups : objectif 3♥ = '+s.path.length+' coup(s). Solution : '+(levels[currentLevel].solution||'déjà sur la sortie')+'</span>';
        return;
      }
    ...
    levels[currentLevel].maxMoves=999;
    updateStats();
    status.innerHTML='<span class="testBad">Calc coups : aucune solution trouvée jusqu’à '+maxDepth+' coups.</span>';
  }catch(e){
    console.error('[CALC] erreur',e);
    status.innerHTML='<span class="testBad">Calc coups : erreur JS - '+(e.message||e)+'</span>';
  }
}
...
function testCurrentLevel(){
  try{
    ...
    if(start.dead){
      status.innerHTML='<span class="testBad">Test du jeu : impossible dès le départ ('+start.reason+').</span>';
      return;
    }
    ...
    while(q.length){
      ...
      if(s.p.x==s.ex.x && s.p.y==s.ex.y){
        ...
        levels[currentLevel].solution=s.path.map(d=>d>0?'R':'L').join('');
        updateStats();
        status.innerHTML='<span class="testOk">Test du jeu : OK, finissable en '+s.path.length+' coup(s). Solution : '+(path||"déjà sur la sortie")+'</span>';
        return;
      }
      if(s.path.length>=maxDepth)continue;
      for(let d of [-1,1]){
    ...
    status.innerHTML='<span class="testBad">Test du jeu : aucune solution trouvée jusqu’à '+maxDepth+' coups. Niveau probablement bloquant.</span>';
  }catch(e){
    console.error('[TEST] erreur',e);
    status.innerHTML='<span class="testBad">Test du jeu : erreur JS - '+(e.message||e)+'</span>';
  }
}
...
async function loadStaticLevels(){
  // V57 STRICT EXTERNAL : aucun niveau embarqué, aucun fallback localStorage.
  // On teste plusieurs chemins pour éviter le gag du fichier au mauvais niveau.
  try{
